...
void continueExecuting( int out_fd ) {
    char buffer[2] = "C\n";
    write( out_fd, buffer, 2 );
}

int getResult( int x, int y, int out_fd, int in_fd, char buffer[10] ) {
    sprintf( buffer, "%i\n", x );
...
void findTractorBeam( int in_fd, int out_fd ) {
    ...
    while( 1 ) {
        for( int j = i; j >= 0; j--  ) {
            int res = getResult( j, i, out_fd, in_fd, buffer );
            if( res == 1 ) {
                j -= 99;
                if( j < 0 )
                    break;
                continueExecuting( out_fd );
                res = getResult( j, i, out_fd, in_fd, buffer );
                if( res == 0 )
                    break;
                i += 99;
                continueExecuting( out_fd );
                ...
                i -= 99;
                if( res == 1 ) {
                    printf( "WILL FIT AT: X=%i, Y=%i, RESULT=%i\n", j, i, j*10000+i );
                    return;
                }
                break;
            }
