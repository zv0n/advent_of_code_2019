...
int main() {
    ...
    FILE *input_pipe = fdopen( pipes_out[0], "r" );
    ...
    for ( int i = 0; i < 120; i++ ) {
        ...
        phase_input[1] = '\n';
        write( pipes45[1], phase_input, 2 );
        snprintf( phase_input, 2, "%i\n", phase[1] );
        phase_input[1] = '\n';
        write( pipes34[1], phase_input, 2 );
        snprintf( phase_input, 2, "%i\n", phase[2] );
        phase_input[1] = '\n';
        write( pipes23[1], phase_input, 2 );
        snprintf( phase_input, 2, "%i\n", phase[3] );
        phase_input[1] = '\n';
        write( pipes12[1], phase_input, 2 );
        snprintf( phase_input, 2, "%i\n", phase[4] );
        phase_input[1] = '\n';
        ...
        size_t thrust;
        while( (pipe_read = getline( &pipe_input, &pipe_size, output )) > 0 ) {
            if( !strcmp( pipe_input, "END\n" ) )
                break;
            write( pipes_out[1], pipe_input, pipe_read );
            thrust = strtoul( pipe_input, NULL, 10 );
        }
        size_t phase_num = phase[0] + 10 * phase[1] + 100 * phase[2] +
                           1000 * phase[3] + 10000 * phase[4];
        getline( &pipe_input, &pipe_size, input_pipe );
        if ( thrust > maxthrust ) {
            maxthrust = thrust;
            maxphase = phase_num;
        }
