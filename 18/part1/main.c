...
int main( void ) {
    ...
    for ( size_t i = 0; i < input_lines; i++ ) {
        for ( size_t j = 0; j < input_len; j++ ) {
            ...
            if ( input_graph[i][j] >= 97 && input_graph[i][j] <= 122 ) {
                keys[key_count][0] = j;
                keys[key_count][1] = i;
                keys[key_count][2] = input_graph[i][j];
                key_count++;
            }
        }
    }
    ...
    for ( int i = 0; i < key_count; i++ ) {
        key_paths[i] = graphPath( input_graph, input_len, input_lines, start_x,
                                  start_y, keys[i][0], keys[i][1], graph_info,
    ...
    printf( "MINIMUM PATH TO GET ALL KEYS: %zu\n",
            shortestPathRec( key_paths, key_key_paths, 0, 1, available_keys,
                             found_keys, key_requirements, key_count, 0 ) );
    for( int i = 0; i < key_count; i++ ) {
