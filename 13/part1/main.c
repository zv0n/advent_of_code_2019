...
void findExtremes( size_t **pixels, size_t len_x, size_t len_y, size_t *max_x, size_t *max_y ) {
    *max_x = 0;
    *max_y = 0;
...
void printIt( size_t **pixels, size_t max_x, size_t max_y, bool ret ) {
    size_t width;
    size_t height;
    ...
    if( ret ) {
        printf( "\x1b[%zuD\x1b[%zuA", width, height );
        usleep( 6000 );
    }
}

size_t playGame( int in_fd ) {
    FILE *f_in = fdopen( in_fd, "r" );
    char *input = NULL;
    size_t input_len = 0;
    size_t max_x = 128;
    size_t max_y = 128;
    size_t **pixels = calloc( max_y, sizeof( size_t* ) );
    ...
    while( 1 ) {
        ...
        if( input[0] == 'E' )
            break;
        size_t x = strtoul( input, NULL, 10 );
        getline( &input, &input_len, f_in );
        size_t y = strtoul( input, NULL, 10 );
        getline( &input, &input_len, f_in );
        size_t tile = strtoul( input, NULL, 10 );
        
        if( x >= max_x ) {
            for( size_t i = 0; i < max_y; i++ ) {
                size_t *tmp = realloc( pixels[i], (x+1) * sizeof( size_t ) );
                if( tmp == NULL )
                    error( EXIT_FAILURE, errno, "realloc" );
                pixels[i] = tmp;
                for( size_t j = max_x; j < x+1; j++ ) {
                    pixels[i][j] = 0;
                }
            }
            max_x = x+1;
        ...
        if( y >= max_y ) {
            size_t **tmp = realloc( pixels, (y+1) * sizeof( size_t*  ) );
            if( tmp == NULL )
                error( EXIT_FAILURE, errno, "realloc" );
            pixels = tmp;
            for( size_t i = max_y; i < y+1; i++ ) {
                pixels[i] = calloc( max_x, sizeof( size_t ) );
            }
            max_y = y+1;
        }
